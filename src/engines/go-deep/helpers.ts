...
export const ENGINE_NAME = 'go-deep' as const

export const RULE_CATEGORIES: Record<string, Category> = {
...
export const RULE_SEVERITIES: Record<string, Severity> = {
...
export const IO_CALL_RE = new RegExp(
  `\\b(?:${[...ERROR_RETURNING_PACKAGES].join('|')})\\s*\\.`,
)

export const SIDE_EFFECT_RE = new RegExp(
...
export function extractGenericParams(text: string): Set<string> {
...
export function getFunctionSignatureStart(text: string): { name: string; index: number } | null {
...
export function extractParamList(lines: CodeLine[], startIdx: number): { text: string; startLine: number } | null {
...
export function findFunctionBodyRange(lines: CodeLine[], startIdx: number): { start: number; end: number } | null {
  let bodyStartLine = -1
  let bodyStartCol = -1
...
export function findTopLevelFunctions(lines: CodeLine[]): { start: number; end: number; bodyStart: number }[] {
...
export function getValueType(param: string, genericParams: Set<string>): string | null {
  if (
...
export function collectFiles(context: EngineContext): Promise<string[]> {
