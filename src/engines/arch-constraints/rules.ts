...
import type { ImportInfo } from "../../utils/file-utils.js"
...
export function countExports(lines: { num: number; text: string }[]): number {
  let count = 0;
...
export function reportCircularDependencies(
...
export function detectUnstableDependencies(
  imports: ImportInfo[],
  filePath: string,
): Diagnostic[] {
  const results: Diagnostic[] = [];
